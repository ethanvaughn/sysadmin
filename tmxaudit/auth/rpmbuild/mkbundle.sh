...
function usage_exit {
	...
	echo
	echo "    eg."
	echo "    mkbundle.sh \"evaughn@10.24.74.9\" ./rpmbuild/auth-host"
	echo
	exit 1
}
...
srcdirs=$(ls -1 .. | egrep -v 'CVS|rpmbuild|test')
mkdir -p ./${proj}-${rpmver}
for i in $srcdirs; do cp -R ../$i ./${proj}-${rpmver}/; done
find ./${proj}-${rpmver} -type d | grep CVS | xargs rm -rf 
cp httpd/conf.d/tmxaudit.conf ./${proj}-${rpmver}/
cp ssh/id_rsa.pub ./${proj}-${rpmver}/ 
