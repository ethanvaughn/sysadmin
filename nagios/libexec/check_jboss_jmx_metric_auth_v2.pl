...
#
# $Id: check_jboss_jmx_metric_auth_v2.pl,v 1.2 2009/07/16 23:07:47 evaughn Exp $
# $Date: 2009/07/16 23:07:47 $
#
###############################################################################################
...
sub readThresholds {
	...
	if ($result) {
		...
		close(FILE);
		if ($arg_count != 6) {
			print 'port, url, realm, username, password and metric are required arguments';
			exit 3;
		}
	} else {
