...
###############################################################################################
#
# Name:		check_logserver_health.pl
# Purpose:	Using Tomax's API, connects to TMX LogServer and queries its status
#
# $Id: check_logserver_health.pl,v 1.1 2008/06/30 18:57:20 jkruse Exp $
# $Date: 2008/06/30 18:57:20 $
#
###############################################################################################
...
sub testSocket {
	...
	if ($sock) {
		...
		# Another note: LogServer will barf (the process will just
		# quit running!) unless this API is followed to the letter!
		# the double carriage return is REQUIRED after sending
		# $config{end_string} -- without it, LogServer will just
		# stop running.  (At least as of the version used
		# as of this writing)
		...
		eval {
			...
			alarm($args{t});
			syswrite($sock,$config{begin_string}."\n");
			sysread($sock,$response,1024);
			syswrite($sock,$config{end_string}."\n\n");
			close($sock);
			alarm(0);
		};
		if ($@) {
			# The socket connected, but an error was thrown during one of the various read/write ops
			if ($@ =~ /timeout/) {
				$output = sprintf('Timeout waiting for LogServer Response (Port %d)',$config{port});
				$status_code = 2;
			} else {
			...
			if ($response =~ /$config{response_string}/) {
				$output = sprintf('LogServer Status OK');
				$status_code = 0;
			} else {
				$output = sprintf('LogServer Error: Restart Recommended');
				$status_code = 2;
			}
		}
	} else {
		# This means the socket failed to connect
		if ($!) {
			my $error = $!;
	...
	if ($result) {
		while(<FILE>) {
			...
			if (/^port:(\d+)$/) {
				...
				$arg_count++;
				next;
			} elsif (/^(begin|end|response)_string:(.*)/) {
				$config{$1.'_string'} = $2;
				$arg_count++;
				next;
			} elsif (/(.*):(.*)/) {
		...
		close(FILE);
		if ($arg_count != 4) {
			print 'port, begin_string, end_string, and response_string are required arguments';
			exit 3;
		}
	} else {
